...
//         fVerbose=TRUE;
         break;
      case 'P':
         if (iPort==0) {ParamsPhysDma=0; break;}
         if (iPort==1) {ParamsPhysDma=1; break;}
         return FALSE;
      default:
...
//        DevHelp_ResetTimer((NPFN)TimerHandler);
    ...
    StackIDC(IDC_CREATE_HOST,&fpGlobalHostOps);

    pOhciDriver=new OHCIDRIVER;
    if (pOhciDriver==NULL)
    {
        ddprintf("ERROR: pOHCIDRIVER is NULL\n");
        return;
    }
    pOhciDriver->pHostOps=fpGlobalHostOps;
...
//    if (pGlobalStack==NULL) ddprintf("ERROR: pFireStack is NULL\n");
    ddprintf("heapSize %d\n",_memfree());
    ddprintf("Allocating hooks for dma\n");
    DevHelp_AllocateCtxHook((NPFN)DMARTaskletRoutine,&DMARTasklet);
    DevHelp_AllocateCtxHook((NPFN)DMATTaskletRoutine,&DMATTasklet);
